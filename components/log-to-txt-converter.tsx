...
import { UploadIcon, DownloadIcon, FileIcon, AlertCircle, XIcon, TrashIcon } from 'lucide-react'
...
export function LogToTxtConverter() {
  ...
  const downloadAllFiles = async () => {
    try {
      ...
      setError("An error occurred while downloading all files. Please try again.")
    }
  }
          ...
          <CardContent>
            <div className="space-y-4">
              {convertedFiles.length > 0 ? (
                ...
                <p className="text-center text-muted-foreground">No converted files yet.</p>
              )}
              {error && (
